...
export const LoginPage = () => {
  ...
  const processLogin = async () => {
    try {
      ...
      if (!response) {
        addAlert('Не удалось войти в аккаунт', AlertStatus.Error, AlertPosition.TopRight);
        return;
      }
      ...
      if (
        ...
        message.includes('invalid credentials') ||
        message.includes('Неверные учетные данные')
      ) {
        triggerShake('email', 'Неверный email или пароль');
        triggerShake('password', 'Неверный email или пароль');

        addAlert('Неверный email или пароль', AlertStatus.Error, AlertPosition.TopRight);
        return;
      }

      addAlert('Произошла ошибка при входе', AlertStatus.Error, AlertPosition.TopRight);
      console.error(error);
    } finally {
      stopLoading();
    }
  ...
  const handleSubmit = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    const hasErrors = emailMessage.length > 0 || passwordMessage.length > 0;
          ...
          <div className="auth-page__forms-container">
            <div className="auth-form__header">
              <div className="auth-form__title-block">
                <Title1 className="auth-form__title">Вход в аккаунт</Title1>

                <Text className="auth-form__subtitle">
                  Войдите, чтобы продолжить работу в редакторе
                </Text>
              </div>
            </div>
            ...
            <form className="auth-form" onSubmit={handleSubmit}>
              ...
              <div className="auth-form__footer">
                <Text className="auth-form__footer-text">Нет аккаунта?</Text>

                <Link href={routeNames.REGISTER_PAGE} className="auth-form__footer-link">
                  Зарегистрироваться
                </Link>
              </div>
            </form>
